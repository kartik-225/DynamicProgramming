#include <bits/stdc++.h>
using namespace std;

#define cout(a) cout << a;

// Shorthand Values and Functions Names
#define max_int INT_MAX
#define min_int INT_MIN
#define INF 1e8
#define endl "\n"
#define pb push_back
#define pop pop_back

// Shorthand for-loop 
#define fori(a,b,n) for(int a = b ; a <= (n) ; a++)
#define ford(a,b,n) for(int a = b ; a >= (n) ; a--)

// Shorthand Array Methods
#define lbArray(arr , n , k) lower_bound(arr , arr + n , k) - arr.begin()
#define ubArray(arr , n , k) upper_bound(arr , arr + n , k) - arr.begin()
#define sortArray(arr , n) sort(arr , arr + n)
#define sumArray(arr , n) accumulate(arr , arr + n , 0)
#define maxArray(arr , n) *max_element(arr , arr + n)
#define minArray(arr , n) *min_element(arr , arr + n)


// Shorthand Vector Methods
#define lbVector(arr , k) lower_bound(arr.begin() , arr.end() , k) - arr.begin()
#define ubVector(arr , k) upper_bound(arr.begin() , arr.end() , k) - arr.begin()
#define sortVector(arr) sort(arr.begin(), arr.end())
#define sumVector(arr) accumulate(arr.begin() , arr.end() , 0)
#define maxVector(arr) *max_element(arr.begin() , arr.end())
#define minVector(arr) *min_element(arr.begin() , arr.end())


// Shorthand Containers and Datatypes
typedef long long ll;
typedef pair<int, int> ii; 
typedef vector<ii> vii;
typedef vector<vector<int> > vvi;
typedef vector<int> vi;
typedef map<int,int> mii;
typedef unordered_set<int> ust;
typedef set<int> st;


// For taking input from external file and showing output in external file
void cp_function(){
	ios::sync_with_stdio(0); 
	cin.tie(0);
	cout.tie(0);

	#ifndef ONLINE_JUDGE
		freopen("input.in" , "r" , stdin);
		freopen("output.in" , "w" , stdout);
	#endif
}


int Recursive(vvi triangle , int row = 0 , int col = 0){
	if(row == triangle.size()-1) return triangle[row][col];

	int down = triangle[row][col] + Recursive(triangle , row+1 , col);

	int diagonalDown = triangle[row][col] + Recursive(triangle , row+1 , col+1);

	return min(down , diagonalDown);
}




int Memoization(vvi triangle , vvi dp , int row = 0 , int col = 0){
	if(row == triangle.size()-1) return triangle[row][col];

	if(dp[row][col] != -1) return dp[row][col];

	int down = triangle[row][col] + Memoization(triangle , dp , row+1 , col);

	int diagonalDown = triangle[row][col] + Memoization(triangle , dp , row+1 , col+1);

	dp[row][col] = min(down , diagonalDown);

	return dp[row][col];
}


int Tabulation(vvi triangle , int n){
	vvi dp(n+1 , vi(n+1 , 0));

	fori(i , 0 , n) dp[n][i] = triangle[n][i];
	
	ford(i , n-1 , 0){
		ford(j , i , 0){
			int down = triangle[i][j] + dp[i+1][j];
			int diagonalDown = triangle[i][j] + dp[i+1][j+1];
			dp[i][j] = min(down , diagonalDown);
		}
	}

	return dp[0][0];
}



int Tabulation_Optimised(vvi triangle , int n){
	vvi dp(n+1 , vi(n+1 , 0));

	vi prev(n+1 , 0) , current(n+1 , 0); 

	fori(i , 0 , n) prev[i] = triangle[n][i];
	
	ford(i , n-1 , 0){
		ford(j , i , 0){
			int down = triangle[i][j] , diagonalDown = triangle[i][j];
			down = triangle[i][j] + prev[j];
			diagonalDown = triangle[i][j] + prev[j+1];
			current[j] = min(down , diagonalDown);
		}
		prev = current;
	}

	return prev[0];
}




int main(){

	cp_function();

	vvi triangle = {{1},
					{2,3},
					{3,6,7},
					{8,9,6,10}};	


	int rows = triangle.size();
	int cols = rows;

	// cout(Recursive(triangle));

	// vvi dp(rows , vi(rows , -1));
	// cout(Memoization(triangle , dp));

	cout(Tabulation(triangle , rows-1));

	// cout(Tabulation_Optimised(triangle , rows-1));

	return 0;
}	