...
int Recursive(vi price , int n , int buy , int fee , int i = 0){
	if(i == n) return 0;

	int profit = 0;

	if(buy)
		profit = max(-price[i] + Recursive(price , n , 0 , fee , i+1) ,
					Recursive(price , n , 1 , fee , i+1));
	else
		profit = max(price[i] - fee + Recursive(price , n , 1 , fee , i+1) , 
					Recursive(price , n , 0 , fee , i+1));
...
int main(){
	...
	vi stocks = {1,3,2,8,4,9};
	int n = stocks.size();
	int fee = 2;


	cout << Recursive(stocks , n , 1 , fee);
