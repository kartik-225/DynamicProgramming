...
int Recursive(vi price , int n , int buy , int limit , int i = 0){
	if(limit == 0) return 0;
	if(i == n) return 0;
	...
	if(buy)
		profit = max(-price[i] + Recursive(price , n , 0 ,limit , i+1) ,
					Recursive(price , n , 1 , limit ,i+1));
	else
		profit = max(price[i] + Recursive(price , n , 1 , limit-1 , i+1) , 
					Recursive(price , n , 0 , limit , i+1));
